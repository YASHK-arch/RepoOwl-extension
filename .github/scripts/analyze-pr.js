...
- If is_spam=true OR is_prompt_injection=true: use ONLY ["invalid", "spam"] or ["invalid", "Prompt Injection"] from the allowed list. No topic labels.
- If recommended_action="APPROVE": pick at most 3 topic labels that EXACTLY match names in the allowed list. Do NOT invent labels.
...
`;
  ...
  // 6. Filter LLM-suggested labels: only use labels that the maintainer
  //    has pre-created in the repo. Never auto-create labels from AI output.
  //    (existingLabelNames was fetched earlier in step 2d)
  ...
  for (const label of analysis.suggested_labels || []) {
    if (existingLabelSet.has(label.toLowerCase())) {
      ...
      console.log(`  Skipping AI-suggested label '${label}' - not in repo labels.`);
    }
  }
